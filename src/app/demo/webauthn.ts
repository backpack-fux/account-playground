...
export class WebAuthnCredentials {
  ...
  public create({
    ...
    const userVerificationFlag =
      userVerification === UserVerificationRequirement.required ? 0x04 : 0x01;
    ...
    const credential = publicKey.allowCredentials
      .flatMap(({ id }) =>
        this.#credentials.filter(
          (c) => c.rp === publicKey.rpId && c.id === ethers.hexlify(id)
        )
      )
      .at(0);
 ...
 */
export function extractPublicKey(response: AuthenticatorAttestationResponse): {
  x: bigint;
  y: bigint;
} {
  ...
  const cosePublicKey = attestationObject.authData.slice(
